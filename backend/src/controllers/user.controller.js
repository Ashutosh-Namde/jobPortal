...
const registrationController = async (req, res) => {
  try {
    ...
    if (!fullname || !email || !password || !phoneNumber || !role) {
      return res
        .status(400)
        .json({ message: "fill all the requirements", success: false });
    }
    ...
    return res
      ...
      .json({
        message: `Account Created Successfully  ${user.fullname} `,
        success: true,
        user,
      });
  } catch (error) {
...
const updateProfileController = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ message: `Account updated succesfuly`, user, success: true });
  } catch (error) {
    console.log(error);
